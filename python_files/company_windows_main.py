...
def execute_crawling(username, password, companies_excel_path,  cookies_path,  employees_json_path):
...
if __name__ == '__main__':
    ...
    while dont_stop:
        p = Process(target=execute_crawling, args=(username, password, companies_excel_path,  cookies_path,  employees_json_path))
        p.start()
        p.join()
        ...
        os.system('start EXCEL.EXE "%s"' % companies_excel_path)
