...
class LinkedinSpider(InitSpider):
    ...
    profiles_search_parsed = 0

    def parse_profile(self, response):
        self.profiles_search_parsed += 1

        if self.profiles_search_parsed > 15:
            print()
            checkprint('Esta é uma versão de testes. Por segurança, aperte CRTL + C para encerrar, pois já foram acessados 15 perfis de funcionários.\n')
            raise CloseSpider('Spider encerrado manualmente')
